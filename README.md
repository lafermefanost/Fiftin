...
**Compression des photos avant envoi.** Chaque nouvelle photo choisie (maison ou chambre) passe par `compressImageFile()` avant d'entrer dans `hfSelectedFiles`/`unit.photos` : redimensionnée à 1920px sur son plus grand côté (au-delà, aucune perte visible pour cet usage, même en rétina) et recompressée en JPEG qualité 0.85 via `<canvas>`. Réduit le poids des pages (fil, fiche détail) et le coût de stockage/bande passante Firebase, sans changer le pipeline d'upload : le résultat reste un `File` classique (juste renommé en `.jpg`), donc `resolvePhotoEntries()`/`uploadPhotoFiles()` n'ont rien à savoir de ce qui s'est passé avant. Si la compression échoue (image non décodable, `<canvas>` indisponible) ou n'apporte rien (image déjà petite/optimisée), le fichier original est envoyé tel quel plutôt que de bloquer le dépôt.
