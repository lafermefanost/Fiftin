...
Même logique sur la **carte du fil** (avant même d'ouvrir la fiche) : une annonce à plusieurs chambres n'affiche que « Voir plus » (pas de bouton « Vérifier la disponibilité », pas de panneau de calendrier caché dans le DOM) — vu qu'il n'y a pas de planning général à vérifier, il faut d'abord choisir une chambre sur la fiche détail. Une annonce à une seule chambre garde les deux boutons, comportement inchangé.

Changer d'onglet conserve la position de scroll (pas besoin de rescroller à chaque fois) ; choisir une chambre depuis sa carte en Vue d'ensemble y bascule directement. Les onglets eux-mêmes reprennent le principe des onglets de navigateur plutôt que des pilules isolées (`.detail-tab`, coins arrondis en haut seulement) : chaque onglet inactif porte sa propre ligne de base grise (séparé du contenu, fond `--cream-deep` discret), l'onglet actif passe en couleur pleine — `--sauge` avec texte `--cream`, même convention que l'élément actif d'une rangée d'onglets/boutons dans app/ (`.mbnav-btn.on`/`.vtab.on`, voir la règle de DA en tête de ce document) — et sa ligne de base disparaît (même couleur que son propre fond) pour se fondre directement dans le contenu juste en dessous : on voit à la fois QUEL onglet est ouvert (couleur) et QU'IL est bien connecté au contenu sous lui (pas de séparation). Comportement strictement inchangé pour une annonce sans plusieurs chambres (0 ou 1) : aucun onglet, même code qu'avant.

**Carte globale + cartes blanches à ombre portée, sur la fiche détail.** Reprend le CSS exact de la vue Année de `app/`, pas juste le principe : dans `app/`, chaque structure (`.yr-group` dans `renderYear()`) est UN conteneur blanc bordé (`background:#fff;border-radius:12px;padding:12px 14px;border:1.5px solid var(--light)` où `--light:#E2DFD8`) qui englobe tous ses sous-encarts (`.tsec-panel`, via le helper `subSection()` — CA/occupation en tuiles `.mstat-card` bordées, puis Seuils/Revenus par mois/Top clients/Répartition chacun dans son propre `.tsec-panel`, `background:var(--white);border-radius:10px;overflow:hidden;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,.12)`) — fond blanc sur fond blanc, donc pas de bordure ni d'espace visible entre eux, seule une légère ombre portée les détache du conteneur qui les entoure. Séjours reprend cette structure à l'identique : `.detail-group` (= `.yr-group`, mêmes valeurs : fond `#fff`, rayon 12px, padding 12px 14px, bordure `1.5px solid #E2DFD8`) juste sous les onglets, englobe TOUT le contenu de l'onglet actif — `.detail-card` (= `.tsec-panel` : fond `#fff`, rayon 10px, `margin-bottom:12px`, `box-shadow:0 2px 8px rgba(0,0,0,.12)`, `.detail-card>*:last-child{margin-bottom:0}` pour éviter le double espacement en bas) pour chaque section (stats, description, Prestations, Chambres/Créneaux, Votre hôte), et même le panneau de demande rapide (calendrier) est maintenant à l'intérieur de `.detail-group`, pas à côté. `.host-card` a perdu son fond `--cream-deep` (devenu transparent) pour ne pas empiler une carte dans la carte.
