...
**Carte blanche par section, sur la fiche détail.** Même principe que la vue Année de `app/` (chaque bloc — CA, occupation, seuils, graphique — est sa propre carte blanche posée sur le fond crème, plutôt que du texte qui s'enchaîne à plat) : le contenu de la fiche détail est découpé en blocs `.detail-card` (fond `#fff`, coins arrondis, `.detail-card>*:last-child{margin-bottom:0}` pour éviter le double espacement en bas de carte). Sur la Vue d'ensemble : une carte pour le prix/distance/délai de réponse, une pour avis Google + description, une pour Prestations, une pour la liste des Chambres, une pour Votre hôte. Sur l'onglet d'une chambre : une carte titre+prix/capacité/distance, une pour Prestations, une pour Créneaux libres — puis Votre hôte, sa propre carte, commune aux deux vues. `.host-card` a perdu son fond `--cream-deep` (devenu transparent) pour ne pas créer une carte dans la carte. Le panneau de demande rapide (calendrier, déjà sa propre carte blanche depuis le redesign calendrier) garde son style existant, non touché.
